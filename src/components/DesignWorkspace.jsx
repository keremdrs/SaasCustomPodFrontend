...
import PrintfulEdmWorkspace  from './PrintfulEdmWorkspace';

// ── FLUX en-boy oranları ─────────────────────────────────
const ASPECT_OPTIONS = [
  { id: '1:1',  label: '1:1',  width: 1344, height: 1344 },
  { id: '4:3',  label: '4:3',  width: 1536, height: 1152 },
  { id: '3:4',  label: '3:4',  width: 1152, height: 1536 },
  { id: '16:9', label: '16:9', width: 1536, height: 864  },
...
export default function DesignWorkspace({
  ...
  const [cachedPrintB64,     setCachedPrintB64]     = useState(null);
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      const enriched = await Promise.all(list.map(async p => {
        // Printful ürünleri için blueprint_templates sorgusu gerekmez
        if (p.fulfillment === 'printful') return p;
  ...
  // ── Ürün değişince aspect öneri ─────────────────────────
  useEffect(() => {
    if (selectedProduct?.print_width && selectedProduct?.print_height) {
      setSelectedAspect(suggestAspect(selectedProduct.print_width, selectedProduct.print_height));
    }
  }, [selectedProduct?.id]);
  ...
  const handleAddLayerFile = async (e) => {
    ...
    if (!file.type.startsWith('image/')) { setError('Sadece görsel dosyaları kabul edilir.'); return; }
    setAddingLayer(true);
    setError('');
    try {
      const ext  = file.name.split('.').pop().toLowerCase();
      const path = `layers/${userId}/${activeOrder.id}_${Date.now()}.${ext}`;
      const { error: upErr } = await supabase.storage.from('orders').upload(path, file, { contentType: file.type, upsert: false });
      if (upErr) throw upErr;
      const { data: { publicUrl } } = supabase.storage.from('orders').getPublicUrl(path);
      if (konvaEditorRef.current?.addLayer) await konvaEditorRef.current.addLayer(publicUrl);
    } catch (err) {
  ...
  const handleConfirm = async () => {
    if (profile.credits < CREDIT_COST.mockup) {
      setError(`Mockup oluşturmak ${CREDIT_COST.mockup} kredi gerektiriyor.`); return;
    }
    setError('');
    let b64;
    try {
      b64 = await generatePrintFile();
      setCachedPrintB64(b64);
    } catch (err) {
      setError('Tasarım dosyası alınamadı: ' + err.message); return;
    }
  ...
  // ── Ürün seçici UI yardımcısı ────────────────────────────
  const ProductSelector = () => (
    <>
      {sellerProducts.length > 1 && (
        <div style={{ marginBottom: 16 }}>
          <div className="label" style={{ marginBottom: 8 }}>Hangi ürün için tasarım?</div>
            ...
            {sellerProducts.map(p => {
              ...
              return (
                <button key={p.id} type="button"
                  ...
                  style={{
                    flex:'0 0 auto', minWidth:130, padding:'10px 14px',
                  ...
                  <div style={{ fontSize:11, opacity: isActive ? 0.85 : 0.6 }}>
                    {p.print_width}×{p.print_height}px
                    {p.fulfillment === 'printful' && (
                      <span style={{ color:'#00a881' }}> · Printful</span>
                    )}
                  </div>
                </button>
              );
      ...
      {sellerProducts.length === 1 && selectedProduct && (
        <div style={{ marginBottom:16, padding:'8px 12px', background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:'var(--radius-sm)', fontSize:12, color:'var(--text-muted)' }}>
          🖨️ Tasarım hedefi:{' '}
          <strong style={{ color:'var(--text)' }}>{selectedProduct.name}</strong>
          {' · '}{selectedProduct.print_width}×{selectedProduct.print_height}px
          {selectedProduct.fulfillment === 'printful' && (
            <span style={{ color:'#00a881', marginLeft:6 }}>✨ Printful</span>
          )}
        </div>
      )}
  ...
  return (
    <div className="card" style={{ marginTop: 20 }}>
      ...
      {/* ════════════════════════════════════════════
          PRINTFUL ürünü seçiliyse → EDM Workspace
      ════════════════════════════════════════════ */}
      {isPrintful && (uiState === UI.IDLE || uiState === UI.PROCESSING) && (
        <>
          <ProductSelector />
          <PrintfulEdmWorkspace
            ...
            onClose={() => setSelectedProduct(sellerProducts.find(p => p.fulfillment !== 'printful') || null)}
          />
        </>
      )}

      {/* ════════════════════════════════════════════
          PRİNTİFY workflow (veya Printful değilse)
      ════════════════════════════════════════════ */}

      {/* ── IDLE (Printify) ── */}
      {!isPrintful && uiState === UI.IDLE && (
        <>
          <ProductSelector />
          ...
          {previousImages.length > 0 && (
            <div style={{ marginBottom:20 }}>
              <div className="label" style={{ marginBottom:8 }}>🖼️ Previously Generated Designs</div>
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                {previousImages.map((img, i) => (
                  <div key={i}
                    style={{ position:'relative', cursor:'pointer',
                      border: finalImageUrl === img.url && uiState === UI.READY
                        ? '3px solid var(--brand)' : '2px solid var(--border)',
                      borderRadius:10, overflow:'hidden', width:100, height:100, flexShrink:0 }}
                    onClick={() => {
                      setFinalImageUrl(img.url);
                      setUiState(UI.READY);
                    }}>
                    <img src={img.url} alt={`Prev ${i+1}`}
            ...
            <>
              ...
              <div style={{ marginTop:14 }}>
                <div className="label" style={{ marginBottom:8, display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <span style={{ fontSize:10, color:'var(--text-dim)', background:'var(--bg-hover)', padding:'2px 6px', borderRadius:10 }}>
                    sadece Premium
                  </span>
                </div>
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {ASPECT_OPTIONS.map(a => {
                    ...
                    return (
                      <button key={a.id} type="button" onClick={() => setSelectedAspect(a.id)}
                        style={{
                          position:'relative', minWidth:64, padding:'8px 10px',
                          background: isActive ? 'var(--brand)' : 'var(--bg-card)',
                          color:      isActive ? '#fff' : 'var(--text)',
                          border: isActive ? '2px solid var(--brand)'
                            : isRecommended ? '2px solid var(--success)' : '1px solid var(--border)',
                ...
                {(() => {
                  ...
                  const recommended = selectedProduct
                    ? suggestAspect(selectedProduct.print_width, selectedProduct.print_height) : null;
                    ...
                    <div style={{ fontSize:11, color:'var(--text-dim)', marginTop:8, lineHeight:1.5 }}>
                      ...
                      {mismatch && selectedProduct && (
                        <span style={{ color:'var(--warning)' }}>
                          {' '}· Önerilen: <strong>{recommended}</strong>
                          ({selectedProduct.print_width}×{selectedProduct.print_height}px)
                        </span>
                      )}
                    </div>
              ...
              <div style={{ display:'flex', gap:10, marginTop:12 }}>
                <button className="btn" style={{ flex:1, background:'#374151', color:'#fff' }}
                  onClick={() => handleStartAI('standard')}
                  disabled={profile.credits < CREDIT_COST.standard}>
                  ⚡ Standard — {CREDIT_COST.standard} credits
                </button>
                <button className="btn" style={{ flex:1, background:'#7c3aed', color:'#fff' }}
      ...
      {/* ── PROCESSING (Printify) ── */}
      {!isPrintful && uiState === UI.PROCESSING && (
        <div style={s.center}>
      ...
      {/* ── READY (Printify) ── */}
      {!isPrintful && uiState === UI.READY && finalImageUrl && (
        <>
          {selectedProduct && (
          ...
          <div style={{ marginBottom:16, display:'flex', justifyContent:'center' }}>
            <KonvaDesignArea
              ...
              printArea={product.print_area_x != null ? {
                x: product.print_area_x, y: product.print_area_y,
                w: product.print_area_w, h: product.print_area_h,
              } : null}
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            <button className="btn btn-secondary"
              onClick={() => { setCachedPrintB64(null); setUiState(UI.IDLE); }}>
              ← Regenerate
            </button>
            <button className="btn btn-secondary"
              ...
              onClick={async () => {
                try {
                  ...
                  a.download = `baski_${activeOrder?.etsy_order_no || 'tasarim'}_${product?.print_width ?? ''}x${product?.print_height ?? ''}.jpg`;
                  a.click();
                } catch (err) { setError('İndirme hatası: ' + err.message); }
              }}>
              ⬇ İndir
            </button>
      ...
      {/* ── CONFIRM ORDER (Printify) ── */}
      {!isPrintful && uiState === UI.CONFIRM_ORDER && (
        <div style={s.center}>
          {isMockupLoading ? (
            <>
            ...
            <>
              ...
              <div style={{ display:'flex', gap:10, flexWrap:'wrap', justifyContent:'center' }}>
                ...
                <button className="btn btn-secondary"
                  ...
                  title={`${product?.print_width}×${product?.print_height}px`}
                  style={{ color:'var(--success)', borderColor:'var(--success)' }}>
                  ⬇ Baskı Dosyası İndir
                </button>
